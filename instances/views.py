...
def index(request):
    ...
    if request.user.is_superuser:
        ...
        instances = Instance.objects.filter(userinstance__user=request.user).prefetch_related(
            "userinstance_set"
        )
...
def stats(request, pk):
    ...
    for blk in blk_usage:
        json_blk.append(
            {"dev": blk["dev"], "data": [int(blk["rd"]) / 1048576, int(blk["wr"]) / 1048576]}
        )

    for net in net_usage:
        json_net.append(
            {"dev": net["dev"], "data": [int(net["rx"]) / 1048576, int(net["tx"]) / 1048576]}
        )
    ...
    if request.method == "POST":
        if request.user.is_superuser or request.user.is_staff or userinstance.is_change:
            ...
            if not request.user.is_superuser and quota_msg:
                ...
                instance.proxy.resize_mem(cur_memory, new_memory)
                msg = _("Memory is resized: %(old)s to %(new)s") % {"old": cur_memory, "new": new_memory}
                addlogmsg(request.user.username, instance.name, msg)
                messages.success(request, msg)
...
def add_new_vol(request, pk):
    ...
    if allow_admin_or_not_template:
        ...
        instance.proxy.attach_disk(
            target_dev, source, target_bus=bus, driver_type=format, cache_mode=cache
        )
...
def add_existing_vol(request, pk):
    ...
    if allow_admin_or_not_template:
        ...
        instance.proxy.attach_disk(
            target_dev, source, target_bus=bus, driver_type=driver_type, cache_mode=cache
        )
...
def add_cdrom(request, pk):
    ...
    if allow_admin_or_not_template:
        ...
        instance.proxy.attach_disk(
            target, "", disk_device="cdrom", cache_mode="none", target_bus=bus, readonly=True
        )
...
def clone(request, pk):
    ...
    if not request.user.is_superuser and quota_msg:
        ...
        r"^([0-9A-F]{2})(:?[0-9A-F]{2}){5}$", clone_data["clone-net-mac-0"], re.IGNORECASE
    ):
